...
export default function Home() {
  ...
  return <div className="shell">
    ...
    <main>
      ...
      {tab==="map"&&<section className="page"><div className="map-layout"><div className="map-card"><div className="map-title"><span>MONASH SPORTS CENTRE</span><b>International Games Night · Floor plan</b></div><div className="floor"><div className="court c1"><strong>BADMINTON COURT 1</strong><span>🟨 Tokyo Tip-Toe</span><span>⭕ Dodo Ring</span><span>🦒 Safari Express</span></div><div className="court c2"><strong>BADMINTON COURT 2</strong><span>🎈 Monsoon Winds</span><span>🥄 Balap Kelereng</span><span>↔️ Golden Order</span></div><div className="court futsal"><strong>FUTSAL FIELD</strong><span>🏝️ Escape the Island</span><span>🪐 Jungle Trek</span></div><div className="court studio"><strong>DANCE STUDIO</strong><span>🥤 Himalayan Hustle</span><span>🎨 Rang-E-Pakistan</span><span>🕳️ Delta Dash</span></div><div className="court lounge"><strong>STUDENT LOUNGE · BUFFERS</strong><span>🪶 Rally Rakyat</span><span>📜 The Endless Scroll</span><span>🌿 The Medicine Room</span></div><div className="court media"><strong>MEDIA ZONE</strong><span>📸 Always open</span></div><div className="entrance">↑ REGISTRATION & ENTRY</div></div></div><div className="route"><span className="eyebrow">TEAM {String(team).padStart(2,"0")} FLOW</span><h3>Your station order</h3>{rotation(team).map((r,i)=><div key={r.game.id}><span>{i+1}</span><p><b>{r.game.name}</b><small>{r.game.venue}</small></p></div>)}</div></div></section>}
    </main>
  </div>;
}

function GameGuide({game}:{game:typeof games[number]}){return <aside className="guide-card" style={{"--accent":game.color} as React.CSSProperties}><div className="guide-hero"><span>{game.icon}</span><small>{game.country}</small><h3>{game.name}</h3><p>{game.venue}</p></div><div className="rules"><span className="eyebrow">HOW TO PLAY</span><ol>{game.rules.map(r=><li key={r}>{r}</li>)}</ol></div></aside>}
