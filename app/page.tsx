...
export default function Home() {
  ...
  useEffect(() => { (async()=>{ try {
    let s; try { s=await supabase("scores?select=team,game,total,notes,breakdown,updated_at"); } catch { s=await supabase("scores?select=team,game,total"); }
  ...
  async function saveScore(){ if(saving)return; setSaving(true); const row={team,game:scoreGame,total:draftTotal,notes:notes.trim(),breakdown:draft,updated_at:new Date().toISOString()}; setScores(v=>[...v.filter(s=>!(s.team===team&&s.game===scoreGame)),row]); try{await supabase("scores?on_conflict=team,game",{method:"POST",body:JSON.stringify(row)});setSync(existingScore?"Score updated":"Saved live")}catch{try{await supabase("scores?on_conflict=team,game",{method:"POST",body:JSON.stringify({team,game:scoreGame,total:draftTotal})});setSync("Saved · notes setup needed")}catch{setSync("Could not save")}}finally{setSaving(false)} }
...
function ScoreRecords({scores}:{scores:ScoreRow[]}){
