...
type ScoreRow = { team: number; game: string; total: number };
...
export default function Home() {
  ...
  useEffect(() => { (async()=>{ try {
    const [s,a,c] = await Promise.all([supabase("scores?select=team,game,total"),supabase("game_availability?select=game,available"),supabase("checklist?select=team,game,complete")]);
  ...
  async function saveScore(){ const row={team,game:scoreGame,total:draftTotal}; setScores(v=>[...v.filter(s=>!(s.team===team&&s.game===scoreGame)),row]); setDraft({}); try{await supabase("scores?on_conflict=team,game",{method:"POST",body:JSON.stringify(row)});setSync("Saved live")}catch{setSync("Saved in preview")} }
  ...
  return <div className="shell">
    <aside className="sidebar">
      <div className="brand"><span className="brand-mark">M</span><div><strong>MUIS IGN</strong><small>International Games Night</small></div></div>
    ...
    <main>
      <header className="topbar"><div><p>JULY ORIENTATION 2026</p><h1>{pages.find(p=>p.id===tab)?.label}</h1></div><div className="header-actions"><span className={`sync ${sync.includes("Live")||sync.includes("Saved live")?"ok":""}`}>{sync}</span><label>Viewing team<select value={team} onChange={e=>setTeam(Number(e.target.value))}>{teams.map(t=><option key={t} value={t}>Team {String(t).padStart(2,"0")}</option>)}</select></label></div></header>
      ...
      {tab==="scoring"&&<section className="page split"><div><div className="panel score-panel"><div className="panel-title"><div><span className="eyebrow">GAME MASTER ENTRY</span><h3>Record a score</h3></div><span>Auto-calculated</span></div><div className="form-grid"><label>Team<select value={team} onChange={e=>setTeam(Number(e.target.value))}>{teams.map(t=><option key={t}> {t}</option>)}</select></label><label>Game<select value={scoreGame} onChange={e=>{setScoreGame(e.target.value);setDraft({})}}>{games.map(g=><option value={g.id} key={g.id}>{g.country} · {g.name}</option>)}</select></label></div><div className="criteria">{selectedGame.criteria.map(([name,max])=><label key={name}><span>{name}<small>Max {max}</small></span><input type="number" min="0" max={max} value={draft[name]??""} onChange={e=>setDraft(v=>({...v,[name]:Number(e.target.value)}))}/></label>)}</div><div className="total"><span>Calculated total<small>Maximum {selectedGame.max} points</small></span><strong>{draftTotal}<i> pts</i></strong></div><button className="primary" onClick={saveScore}>Submit score →</button></div></div><GameGuide game={selectedGame}/></section>}
